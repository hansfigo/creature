...
export const PUT: RequestHandler = async ({request, locals, params}) => {
    ...
    const userId = await db.select({
        id : user.id
    }).from(user).where(eq(user.username, locals.user!.username));    
    ...
    if (formData.profilePicture) {
        try {
            const profilePicture = await useFirebase.uploadFile({ file: formData.profilePicture as File, path: '/users/profilePictures/' });
            data = {
                ...data,
                profilePicture
            }
    ...
    const updatedUser = await db.update(user).set(data).where(eq(user.username, locals.user!.username));
